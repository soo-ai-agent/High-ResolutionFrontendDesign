...
- 외부 알림 채널 없음 → **Slack Webhook + 이메일(SMTP)** — 검토 대기 도착·리뷰 한도 초과·
  브리지 실패·CI 회복 오류(중복 억제)를 발송. 받는 곳은 설정 › 외부 알림 채널에서 입력(DB 영속),
  SMTP 서버는 환경 변수(`SMTP_HOST` 등). 테스트 발송 버튼으로 채널별 성공/실패 즉시 확인.
