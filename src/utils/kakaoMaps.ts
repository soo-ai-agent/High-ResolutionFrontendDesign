...
function readViteEnv(name: keyof ImportMetaEnv): string {
  ...
  const fromProcess: string | undefined = typeof process !== 'undefined' ? process.env[name] : undefined;
...
export async function loadKakaoServices(): Promise<KakaoMapsServices | null> {
  if (!(await loadKakaoMaps())) {
    return null;
  }
  const services: KakaoMapsServices | undefined = typeof window !== 'undefined' ? window.kakao?.maps?.services : undefined;
...
const SDK_LOAD_TIMEOUT_MS: number = 8000;
...
export function loadKakaoMaps(): Promise<boolean> {
  ...
  const jsKey: string = getKakaoJsKey();
  if (!jsKey) {
  ...
  mapsLoading = new Promise<boolean>((resolve) => {
    ...
    const existing: HTMLScriptElement | null = document.querySelector<HTMLScriptElement>('script[data-kakao-maps]');
    if (existing) {
      armTimeout();
    ...
    // libraries=services → 좌표→시군구 역지오코딩(Geocoder.coord2RegionCode) + 키워드 장소검색(Places).
    // https 강제: 평문(http) 페이지에서 프로토콜 상대경로(//)로 로드하면 services 의 dapi(Local) 호출이
    // http 로 나가 Kakao 서버가 400(HTTPS 필요)으로 거부한다 → 장소검색 실패. 항상 https 로 로드한다.
